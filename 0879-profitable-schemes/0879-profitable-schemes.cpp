...
public:
    ...
    int solve(int k,int i,int j,int n,int minProfit,vector<int>& group,vector<int>& profit)
    {
        ...
        notInc=solve(k+1,i,j,n,minProfit,group,profit);
        
       inc=solve(k+1,i+group[k],min(j+profit[k],minProfit),n,minProfit,group,profit);
        
        return dp[k][i][j]=(notInc%mod+inc%mod)%mod;
    }
